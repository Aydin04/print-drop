...
def generate_photo_layout_pdf(
    ...
    crop_data: dict = None,
    is_keychain_mode: bool = False
):
    """
    Arranges 'quantity' copies of cropped photo into paper sheet(s).
    If is_keychain_mode is True, prints paired front/back photos side-by-side or mirrored for folding.
    """
    ...
    # If keychain mode (2-sided), each unit requires 2 photo slots (Depan & Belakang)
    effective_qty = quantity * 2 if is_keychain_mode else quantity
    ...
    total_pages = max(1, math.ceil(effective_qty / items_per_page))
    placed = 0
    ...
    for page_idx in range(total_pages):
        ...
        mode_label = "Gantungan Kunci (2 Sisi)" if is_keychain_mode else "Pas Foto / Cetak"
        header_text = f"AYDIN PRINT | {customer_name} | {mode_label} {photo_w_mm:.1f}x{photo_h_mm:.1f} mm | Total: {quantity} pcs (Hal {page_idx + 1}/{total_pages})"
        if accessory_name:
            header_text += f" | Aksesoris: {accessory_name}"
        c.drawString(margin_left, page_h - (margin_top * 0.65), header_text)

        # Draw grid items
        for row in range(rows):
            for col in range(cols):
                if placed >= effective_qty:
                    break
