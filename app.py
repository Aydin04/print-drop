...
def update_config():
    ...
    if new_data:
        save_db(new_data)
        return jsonify({"status": "success", "message": "Pengaturan & Master Data berhasil disimpan!"})
...
def calculate_price():
    ...
    paper = next((p for p in papers if p["id"] == paper_id), papers[0] if papers else None)
    if not paper:
        return jsonify({"error": "Kertas tidak ditemukan"}), 400
        
    base_price = paper.get("base_selling_price_color", 1000) if color_mode == "COLOR" else paper.get("base_selling_price_bw", 500)
    
    # Duplex logic: if duplex, multiply by (1 + duplex_extra_percent/100)
    if duplex:
        duplex_pct = paper.get("duplex_extra_percent", 80)
        unit_print_price = base_price * (1.0 + (duplex_pct / 100.0))
    else:
    ...
    if mode == "photo_custom":
        ...
        preset = next((pr for pr in db.get("presets", []) if str(pr.get("id")) == str(preset_id)), None)
        
        if preset and "price_per_pcs" in preset:
            print_cost_total = preset["price_per_pcs"] * quantity
            sheets_needed = max(1, math_sheets(paper, preset.get("photo_width_mm", 30), preset.get("photo_height_mm", 40), quantity))
        elif preset and "suggested_price" in preset:
            # e.g. Paket Gantungan Kunci
            print_cost_total = preset["suggested_price"] * quantity
            sheets_needed = max(1, math_sheets(paper, preset.get("photo_width_mm", 40), preset.get("photo_height_mm", 55), quantity))
        else:
            ...
            fits = cols * rows
            sheets_needed = max(1, (quantity + fits - 1) // fits)
    ...
    total_raw = print_cost_total + accessory_price_total
    
    # Rounding according to store settings
    round_to = db.get("settings", {}).get("round_price_to", 500)
    if round_to > 0:
        total_price = int(((total_raw + round_to - 1) // round_to) * round_to)
    else:
        total_price = int(total_raw)

    return jsonify({
        "total_price": total_price,
        "print_cost_total": int(print_cost_total),
        "accessory_price_total": int(accessory_price_total),
        "accessory_name": accessory_name,
        "sheets_needed": sheets_needed,
        "unit_print_price": int(unit_print_price)
    })

def math_sheets(paper, w_mm, h_mm, qty):
    cols = max(1, int(paper["width_mm"] / (w_mm + 3)))
    rows = max(1, int(paper["height_mm"] / (h_mm + 3)))
    fits = max(1, cols * rows)
    return (qty + fits - 1) // fits
...
def upload():
    ...
    if mode == "photo_custom" and uploaded_files and uploaded_files[0].filename:
        ...
        orig_img_path = os.path.join(order_dir, f"original_foto{ext}")
...
"""
    for sf in saved_files:
        struk_content += f"- {sf}\n"
    if generated_pdf:
        struk_content += f"- [PDF SIAP PRINT]: {generated_pdf}\n"
