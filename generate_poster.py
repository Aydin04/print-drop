...
def generate_posters():
    ...
    # 1. QR Wi-Fi 5G (Default)
    wifi_str = "WIFI:S:AYDIN-PRINT-5G;T:WPA;P:aydinprint;;"
    ...
    poster.paste(img_w_resized, (110, b1_y + 85), img_w_resized)

    # Text Step 1
    t1_x = 550
    draw.text((t1_x, b1_y + 100), "1. Scan QR di samping", fill="#ffffff", font_size=30)
    draw.text((t1_x, b1_y + 150), "2. Ketuk 'Hubungkan Wi-Fi'", fill="#38bdf8", font_size=26)
    draw.text((t1_x, b1_y + 220), "Pilihan Nama Wi-Fi Toko:", fill="#cbd5e1", font_size=22)
    draw.text((t1_x, b1_y + 260), "🚀 AYDIN-PRINT-5G (Super Cepat)", fill="#38bdf8", font_size=22)
    draw.text((t1_x, b1_y + 300), "📶 AYDIN-PRINT (Semua Tipe HP)", fill="#94a3b8", font_size=22)
    draw.text((t1_x, b1_y + 355), "🔑 Password : aydinprint", fill="#22c55e", font_size=24)
    draw.text((t1_x, b1_y + 420), "✨ Bebas Kuota • Aman & Cepat", fill="#94a3b8", font_size=20)

    # Box Step 2 (Web Cetak)
    b2_y = 860
    draw.rounded_rectangle([(70, b2_y), (W - 70, b2_y + b_h - 20)], radius=20, fill="#0f172a", outline="#22c55e", width=3)
    ...
    poster.paste(img_u_resized, (110, b2_y + 85), img_u_resized)

    # Text Step 2
    draw.text((t1_x, b2_y + 100), "1. Scan QR di samping", fill="#ffffff", font_size=30)
    draw.text((t1_x, b2_y + 150), "2. Masukkan Nama Pemesan", fill="#22c55e", font_size=24)
    draw.text((t1_x, b2_y + 200), "3. Upload Dokumen / Pas Foto", fill="#22c55e", font_size=24)
    draw.text((t1_x, b2_y + 250), "4. Klik Kirim Pesanan", fill="#22c55e", font_size=24)
    draw.text((t1_x, b2_y + 330), "🌐 Alamat Web : 10.42.0.1:5000", fill="#94a3b8", font_size=22)

    # Footer
    draw.text((W // 2, 1530), "Setelah kirim file, silakan konfirmasi nama Anda ke kasir.", fill="#64748b", anchor="mm", font_size=20)

    poster_path = os.path.join(app_dir, "POSTER_MEJA_AYDIN_PRINT.png")
    poster.save(poster_path, "PNG")
    print(f"Poster Dual-Band tersimpan di: {poster_path}")
